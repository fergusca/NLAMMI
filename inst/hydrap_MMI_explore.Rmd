---
title: "Lake drawdown physical,chemical, & biological characteristics"
author: "C. Emi Fergus"
date: "2024-02-16"
...
#my_comparisons <- list(c("Low","Moderate"), c("Low","High"), #c("Moderate","High"))

#hydrap_mmi_box<-ggplot(dat_red, aes(x=HydrAP_bin,y=MMI_BENT_NLA12, #fill=factor(HydrAP),facet.by=ECOP5_2015))+
#  geom_boxplot(aes(fill=factor(HydrAP_bin),y=MMI_BENT_NLA12),outlier.shape=NA)+
  #geom_violin(aes(fill=factor(HydrAP),y=MMI_BENT_NLA12),outlier.shape=NA)+
#  geom_jitter(width=0.1, alpha=0.2)+
  #stat_compare_means(method="kruskal.test", aes(label=..p.signif..))+
    #stat_compare_means(comparisons=my_comparisons, label="p.signif")+
  #stat_pvalue_manual(stat.test, label = "p.adj.signif", tip.length = 0.01)+
  #stat_compare_means(aes(group=ECOP5_2015),label="p.format")+ #"p.signif"
#  facet_grid(.~ECOP5_2015)+
#  theme(
#    legend.position="none",
#    plot.title=element_text(size=11))+
#  ggtitle("Boxplot benthic MMI & HydrAP")+
#  labs(
#    x=expression("HydAP rank v.9"),
#    y=expression("Macroinvert MMI"))


# Perform mean comparison after grouping the data by ecoregion
#compare_means(MMI_BENT_NLA12~HydrAP_bin, data=dat,
#              group.by = "ECOP5_2015")
...
#bxp_CHLA<-ggplot(
#  dat_red, aes(x=ECOP5_2015, y = CHLA,color=HydrAP_bin))+
#  geom_boxplot()+
...
#bxp_CHLA<-ggplot(
#  dat_red, aes(x=ECOP5_2015, y = CHLA,color=HydrAP_bin,fill=HydrAP_bin))+
#  geom_violin()+
#  scale_y_log10()+
#  theme_classic()
...
## Macroinvert and EI class
```{r, echo=FALSE, warning=FALSE, message=FALSE}
dat_red_ei<-dat_red%>%
  filter(!is.na(EI_class))

#  Boxplot w/ p-values 
bxp_mmi_ei<-ggboxplot(
  dat_red_ei, x="ECOP5_2015", y = "MMI_BENT",
  color="EI_class")#+
  #scale_y_log10()

# Add p-values onto the box plots
stat.test.mmi.ei <-dat_red_ei %>% 
  group_by(ECOP5_2015)%>%
  t_test(MMI_BENT~EI_class)%>%
  adjust_pvalue(method="bonferroni")%>%
  add_significance("p.adj")


stat.test.mmi.ei<-stat.test.mmi.ei%>%
  add_xy_position(x="ECOP5_2015", dodge = 0.8)

hydrap_mmi_ei_box <-bxp_mmi_ei+stat_pvalue_manual(
  stat.test.mmi.ei, label="p", y.position = 90, tip.length=0.01,
  bracket.nudge.y = -2, step.group.by ="ECOP5_2015",step.increase = 0.08)

hydrap_mmi_ei_box
```

## Zooplankton and EI class
```{r, echo=FALSE, warning=FALSE, message=FALSE}
#  Boxplot w/ p-values 
bxp_zoop_ei<-ggboxplot(
  dat_red_ei, x="ECOP5_2015", y = "MMI_ZOOP_NLA6",
  color="EI_class")#+
  #scale_y_log10()

# Add p-values onto the box plots
stat.test.zoop.ei <-dat_red_ei %>% 
  group_by(ECOP5_2015)%>%
  t_test(MMI_ZOOP_NLA6~EI_class)%>%
  adjust_pvalue(method="bonferroni")%>%
  add_significance("p.adj")


stat.test.zoop.ei<-stat.test.zoop.ei%>%
  add_xy_position(x="ECOP5_2015", dodge = 0.8)

hydrap_zoop_ei_box <-bxp_zoop_ei+stat_pvalue_manual(
  stat.test.zoop.ei, label="p", y.position = 90, tip.length=0.01,
  bracket.nudge.y = -2, step.group.by ="ECOP5_2015",step.increase = 0.08)
...
#  Boxplot w/ p-values 
bxp_tn_ei<-ggboxplot(
  dat_red_ei, x="ECOP5_2015", y = "NTL",
  color="EI_class")+
  scale_y_log10()

# Add p-values onto the box plots
stat.test.tn.ei <-dat_red_ei %>% 
  group_by(ECOP5_2015)%>%
  t_test(NTL~EI_class)%>%
  adjust_pvalue(method="bonferroni")%>%
  add_significance("p.adj")
...
stat.test.tn.ei<-stat.test.tn.ei%>%
  add_xy_position(x="ECOP5_2015", dodge = 0.8)

hydrap_tn_ei_box <-bxp_tn_ei+stat_pvalue_manual(
  stat.test.tn.ei, label="p", y.position = 7, tip.length=0.01,
  bracket.nudge.y = -2, step.group.by ="ECOP5_2015",step.increase = 0.08)

hydrap_tn_ei_box
```

## Scatterplots of lake water-level decline and MMI

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Plot MMI vs lake drawdown color coded by E:I class
mmi_dd_scatter<-ggplot(dat_red, aes(x=HORIZDD_use, y=MMI_BENT))+ #,color=EI_class
  geom_point() + 
  geom_smooth(method="lm")+
  scale_x_log10()+
  facet_wrap(ECOP5_2015~.)

mmi_dd_scatter

```

# Scatterplot MMI and E:I by HydrAP class
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Plot MMI vs E:I by HydrAP classclass
mmi_horiz_hydrap_scatter<-ggplot(dat_red, aes(x=HORIZDD_use, y=MMI_BENT, color=HydrAP_bin))+ #
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  scale_x_log10()+
  facet_wrap(ECOP5_2015~.)

mmi_horiz_hydrap_scatter

```

# Scatterplot TN and E:I by HydrAP class
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Plot 
tn_vert_hydrap_scatter<-ggplot(dat_red, aes(x=VERTDD_use, y=NTL,color=HydrAP_bin))+#
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  scale_y_log10()+
  scale_x_log10()+
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# Plot MMI vs E:I by HydrAP classclass
tn_hydrap_scatter<-ggplot(dat_red, aes(x=E_I, y=NTL,color=HydrAP_bin))+#
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  scale_y_log10()+
  scale_x_log10()+
...
# Summarize lake max depth by ecoregion
zmax<-dat_red%>%
  group_by(ECOP5_2015)%>%
  summarise (mean_zmax=mean(DpthMx_use, na.rm=TRUE), sd=sd(DpthMx_use,na.rm=TRUE))
...
```{r, echo=FALSE, warning=FALSE, message=FALSE,eval=FALSE}
## Table of Lake drawdown condition class by HydrAP class
#Potential hydro-alteration classes are grouped as
#HydrAP <3 = "Low"; HydrAP <= 5 = "Moderate"; HydrAP > 5 = "High"

test<-as.data.frame(table(dat$Drawdown_CONDus15,dat$HydrAP_bin))
test<-test%>%
  pivot_wider(names_from=Var2, values_from = Freq)%>%
  rename(Drawdown=Var1)%>%
  mutate(Drawdown = fct_relevel(Drawdown, "SMALL", "MEDIUM",
                                         "LARGE", "NOT ASSESSED"))


knitr::kable(test, booktabs = TRUE) %>%
  add_header_above(c(" " = 1, "HydrAP class" = 3))#, caption = "Lake Drawdown condition by HydrAP class")
```


## HABITAT and HydrAP - just 0712
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# 
bxp_habitat<-ggboxplot(
  nla0712, x="ECOP5_2015", y = "LitRipCvrQc15",
  color="HydrAP_bin")#+
  #scale_y_log10()

# Add p-values onto the box plots
stat.test.habitat <-nla0712 %>% 
  group_by(ECOP5_2015)%>%
  t_test(LitRipCvrQc15~HydrAP_bin)%>%
  adjust_pvalue(method="bonferroni")%>%
  add_significance("p.adj")


stat.test.habitat<-stat.test.habitat%>%
  add_xy_position(x="ECOP5_2015", dodge = 0.8)

hydrap_habitat_box <-bxp_habitat+stat_pvalue_manual(
  stat.test.habitat, label="p", y.position = 2, tip.length=0.01,
  bracket.nudge.y = -2, step.group.by ="ECOP5_2015",step.increase = 0.08)

hydrap_habitat_box
```

## Exploratory CFA of factors related to water mgmt
```{r, echo=FALSE, warning=FALSE, message=FALSE}
nla0712_dam<-nla0712%>%
  mutate(NIDStorM3_sc = scale(NIDStorM3))%>% # /1000000000
  mutate(elev_relief_m_sc = scale(elev_relief_m))%>% #/100
  mutate(POP_DEN_sc = scale(POP_DEN))%>% #/1000
  mutate(AvgWetIndxWs_sc = scale(AvgWetIndxWs))%>% #/100
  mutate(PctAGR_Cat_sc = PctAGR_Cat/100,
         PctIrrigated.AgLandWs_sc = PctIrrigated.AgLandWs/100,
         PctAgDrainageWs_sc = PctAgDrainageWs/100,
         PctDEVELOPED_Cat_sc = PctDEVELOPED_Cat/100)%>%
  filter(!is.na(damht_zmax_full))

wmgmt.model<-'dam =~ damht_zmax_full + NIDStorM3_sc + elev_relief_m_sc
             agr =~ PctAGR_Cat_sc + PctIrrigated.AgLandWs_sc + PctAgDrainageWs_sc + AvgWetIndxWs_sc
             urb =~ PctDEVELOPED_Cat_sc + POP_DEN_sc + hiiNonAg' #
              

#wmgmt.model<-'urb =~ PctDEVELOPED_Cat + POP_DEN_sc + hiiNonAg'
# agr =~ PctAGR_Cat + PctIrrigated.AgLandWs + PctAgDrainageWs + AvgWetIndxWs
# urb =~ PctDEVELOPED_Cat + POP_DEN + hiiNonAg

fit <- cfa(wmgmt.model, data=nla0712_dam)#,group="ECOP5_2015"
summary(fit, fit.measures=TRUE)
```

# Relationships between horizontal drawdown, habitat, and MMI
```{r, echo=FALSE, warning=FALSE, message=FALSE}
library(GGally)
dat_red2<-nla0712%>%
  select(L_HorizDD_use,LitCvrQc15,LitRipCvrQc15,E_I,MMI_BENT_NLA12)
ggpairs(dat_red2)

mmi_horiz<-ggplot(nla0712,aes(x=ZLOE_HorizDD_use,  y=MMI_BENT_NLA12))+ #HorizDD_use
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  #scale_y_log10()+
  #scale_x_log10()+
...
mmi_litthabitat<-ggplot(nla0712,aes(x=LitCvrQc3OE15,  y=MMI_BENT_NLA12))+ #LitCvrQc15
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  #scale_y_log10()+
  scale_x_log10()+
...
mmi_riphabitat<-ggplot(nla0712,aes(x=LitRipCvrQc15,  y=MMI_BENT_NLA12))+
  geom_point() + 
  geom_smooth(method="lm",se=FALSE)+
  #scale_y_log10()+
  scale_x_log10()+
  facet_wrap(ECOP5_2015~.)
mmi_riphabitat
summary(lm(nla0712$MMI_BENT_NLA12~nla0712$LitRipCvrQc15))
```

```{r, echo=FALSE, warning=FALSE, message=FALSE}

m1<-'ZLOE_HorizDD_use ~ hap_rank_9 + bffFlat_grad + P_WY
     #LitCvrQc15 ~ hap_rank_9 + ZLOE_HorizDD_use + P_WY + LitRipCvrQc15
     LitRipCvrQc15 ~ hap_rank_9 + ZLOE_HorizDD_use + P_WY
     MMI_BENT_NLA12 ~ hap_rank_9 + ZLOE_HorizDD_use + LitRipCvrQc15 + LitCvrQc15 
     
     # Covariance
     #ZLOE_HorizDD_use ~~LitRipCvrQc15
'

fit.sem<-sem(m1, data=nla0712,group="ECOP5_2015")

summary(fit.sem, fit.measures=TRUE)

# request modification indices greater than 3.0 - from Grace USGS materials
mi_min <-modindices(fit.sem)
...
path<-semPaths(fit.sem, title=FALSE, whatLabels = "est",intercepts = FALSE, residuals =FALSE)

#path2 <- mark_sig(path, fit.sem)
#plot(path2)
